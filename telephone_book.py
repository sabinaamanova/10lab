...
def create_table():
    ...
    try:
        config = psycopg2.connect(**params) #параметры для sql
        query = config.cursor()
        query.execute(SQL)
        query.close()
        config.commit() #чтоб выполнялся 1 раз sql
...
def add_user(name, telephone): # чтоб заполнять таблицу
...
def drop_by_name(name): #удаление по имени
...
def show_sort_by_telephone():
    ...
    try:
        ...
        for row in res: #выводить номера ровно
            l = len(row[0])
            print(str(row[0]), end ="")
            for i in range(25-l):
                print(' ', end ="")
